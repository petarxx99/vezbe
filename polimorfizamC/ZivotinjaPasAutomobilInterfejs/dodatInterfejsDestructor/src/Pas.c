...
void Pas_obrisi(Pas **adresaPointeraPsa){
    ...
    *adresaPointeraPsa = NULL;
    printf("Pas sa adrese %lu je obrisan. \n", pasZaBrisanje);
}
...
/* Sada slede metode koju su potrebne da bi se implementirao interfejs MozeProizvestiZvuk  */
...
void Pas_interfejsMozeProizvestiZvukJeObrisan(Pas *_this, void *interfejs){
    if(_this == NULL){
        printf("Pas_interfejsMozeProizvestiZvukJeObrisan je dobio NULL pointer psa. \n", stderr);
        return;
    }

    printf("Pas vlasnika %s je primio poruku da je njegov interfejs MozeProizvestiZvuk sa adrese %lu obrisan.\n",
            _this->vlasnik,
             interfejs);
}
...
MozeProizvestiZvuk* Pas_getMozeProizvestiZvuk(Pas *_this){
    ...
    mozeProizvestiZvuk->primiPorukuDaJeInstancaInterfejsaObrisana = Pas_interfejsMozeProizvestiZvukJeObrisan;
