...
int main(void)
{
  ...
  //  testirajPolimorfizamPsaZivotinje(); /* Polimorfizam, vTable, bitno, uspesno uradjeno. */
    testirajInterfejsMozeProizvestiZvuk(); /* Uspesno implementiran interfejs MozeProizvestiZvuk na psu i automobilu.*/
    return 0;
}

void testirajInterfejsMozeProizvestiZvuk(){
    Pas *pas = Pas_new_sve(4, 20, "Ivan");
    ...
    printf("Koliko puta je proizveo zvuk = %d\n", mozeProizvestiZvuk->getKolikoPutaJeProizveoZvuk(mozeProizvestiZvuk->strukturaKojuMetodeInterfejsaKoriste));
    Interfejs_obrisi(&mozeProizvestiZvuk,
                    mozeProizvestiZvuk->strukturaKojuMetodeInterfejsaKoriste,
                    mozeProizvestiZvuk->primiPorukuDaJeInstancaInterfejsaObrisana);
    ...
    MozeProizvestiZvuk_obrisi(&zvukAutomobil);
    Automobil_delete(&automobil);
